<%@ Page Title="" Language="C#" MasterPageFile="~/Salary/SalaryMaster.master" AutoEventWireup="true" CodeFile="RptQuterlyReport.aspx.cs" Inherits="Salary_Report_RptQuterlyReport" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        function Validation() {
            ...
            if ($('#<%=ddltaxType.ClientID%>').val() == '0') {
                $('#<%=lblMsg.ClientID%>').addClass('alertMsg failed');
                $('#<%=lblMsg.ClientID%>').html('Please Select Tax');
                $('#<%=lblMsg.ClientID%>').show();
                $('#<%=ddltaxType.ClientID%>').css('border-color', 'red');
                $('#<%=ddltaxType.ClientID%>').focus();
                return false;
            }
            else {
                $('#<%=ddltaxType.ClientID%>').css('border-color', 'lightgray');
                $('#<%=ddltaxType.ClientID%>').css('border-color', 'lightgray');
            }
            if ($('#<%=ddlmonthFrom.ClientID%>').val() == '0') {
                $('#<%=lblMsg.ClientID%>').addClass('alertMsg failed');
                $('#<%=lblMsg.ClientID%>').html('Please Select Month From');
                $('#<%=lblMsg.ClientID%>').show();
                $('#<%=ddlmonthFrom.ClientID%>').css('border-color', 'red');
                $('#<%=ddlmonthFrom.ClientID%>').focus();
                return false;
            }
            else {
                $('#<%=ddlmonthFrom.ClientID%>').css('border-color', 'lightgray');
                $('#<%=ddlmonthFrom.ClientID%>').css('border-color', 'lightgray');
            }
            //return true;
            if ($('#<%=ddlmonthTo.ClientID%>').val() == '0') {
                $('#<%=lblMsg.ClientID%>').addClass('alertMsg failed');
                $('#<%=lblMsg.ClientID%>').html('Please Select Month To');
                $('#<%=lblMsg.ClientID%>').show();
                $('#<%=ddlmonthTo.ClientID%>').css('border-color', 'red');
                $('#<%=ddlmonthTo.ClientID%>').focus();
                return false;
            }
            else {
                $('#<%=ddlmonthTo.ClientID%>').css('border-color', 'lightgray');
                $('#<%=ddlmonthTo.ClientID%>').css('border-color', 'lightgray');
            }
            return true;
        }
        ...
        <ContentTemplate>
            ...
            <div class="col-xs-12 col-sm-6 col-sm-offset-3 mrgntp10 mt10">
                <div class="panel panel-primary ">
                    <div class="panel-heading" style="text-align: center">QUATERLY TAX STATMENT</div>
                                    ...
                                    <div class="brdr_box">
                                        ...
                                        <asp:Panel runat="server" ID="Panel1">
                                            <div class="row">
                                                <div class="col-sm-12 col-xs-12">
                                                    <div class="form-group">
                                                        <label class="col-xs-3">Select Tax<i class="text-danger"> *</i></label><div class="col-sm-9">
                                                            <asp:DropDownList runat="server" CssClass="form-control form-control-xs col-sm-9" ID="ddltaxType">
                                                                <asp:ListItem Enabled="true" Text="--Select Tax--" Value="0"></asp:ListItem>
                                                                <asp:ListItem Text="Income Tax " Value="106"></asp:ListItem>
                                                                <asp:ListItem Text="Professional Tax" Value="107"></asp:ListItem>
                                                            </asp:DropDownList>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <asp:Panel runat="server" ID="Panel2">
                                            <div class="row">
                                                <div class="col-sm-12 col-xs-12">
                                                    <div class="form-group">
                                                        <label class="col-xs-3">Month From<i class="text-danger"> *</i></label>
                                                        <div class="col-sm-9">
                                                            <div class="row">
                                                                <div class="col-sm-5">
                                                                    <asp:DropDownList runat="server" OnSelectedIndexChanged="ddlmonthFrom_SelectedIndexChanged" AutoPostBack="true" CssClass="form-control form-control-xs col-sm-4" ID="ddlmonthFrom">
                                                                    </asp:DropDownList>
                                                                </div>

                                                                <label class="col-sm-2 control-label">To<i class="text-danger"></i></label>

                                                                <div class="col-sm-5">
                                                                    <asp:DropDownList runat="server" OnSelectedIndexChanged="ddlmonthTo_SelectedIndexChanged" AutoPostBack="true" CssClass="form-control form-control-xs col-sm-4" ID="ddlmonthTo">
                                                                    </asp:DropDownList>
                                                                </div>
                                                            </div>
                                                        </div>
